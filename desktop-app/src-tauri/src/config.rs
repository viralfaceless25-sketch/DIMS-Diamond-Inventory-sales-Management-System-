...
pub fn production_url() -> tauri::Url {
    ...
    assert!(url.host_str().is_some(), "production desktop URL must have a host");
    url
}
...
mod tests {
    ...
    fn navigation_stays_on_the_inventory_origin() {
        assert!(is_allowed_navigation(&tauri::Url::parse(
            "https://maitri-inventory-web.onrender.com/login/"
        ).unwrap()));
        assert!(!is_allowed_navigation(&tauri::Url::parse(
            "https://example.com/"
        ).unwrap()));
        assert!(!is_allowed_navigation(&tauri::Url::parse(
            "http://maitri-inventory-web.onrender.com/"
        ).unwrap()));
    }
