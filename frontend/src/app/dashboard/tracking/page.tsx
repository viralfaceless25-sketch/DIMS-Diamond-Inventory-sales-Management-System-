    ...
    <>
      ...
      <div style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: 26 }}>
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 9, padding: '9px 12px', minWidth: 220, flex: '1 1 220px' }}>
            ...
            <input
              ...
              autoComplete="off"
              style={{ flex: 1, minWidth: 0, background: 'none', border: 'none', outline: 'none', color: t.text, font: "500 14.5px 'Inter'" }}
            />
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 9, padding: '9px 12px', minWidth: 220, flex: '1 1 220px' }}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={t.textFaint} strokeWidth="2"><circle cx="11" cy="11" r="7" /><path d="M21 21l-4-4" /></svg>
        ...
        <div style={{ background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 12, overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: summaryCols, gap: 10, padding: '11px 16px', minWidth: 980, font: "700 11.5px 'Inter'", color: t.textFainter, letterSpacing: '0.04em' }}>
          ...
          {loading ? (
            ...
            return (
              <div key={row.id} style={{ borderTop: `1px solid ${t.rowBorder}` }}>
                ...
                >
                  <Copyable value={row.barcode} style={{ font: "800 14.5px 'JetBrains Mono'", color: ACCENT }} />
                  <div style={{ font: "600 13.5px 'Inter'", color: t.textMuted }}>
                    {row.cert_no ? <Copyable value={row.cert_no} /> : <NonCertBadge />}
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ font: "700 13.5px 'Inter'", color: t.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.rep_name}</div>
                    <div style={{ font: "500 12px 'Inter'", color: t.textFaint }}>Request #{row.request_id}</div>
                  </div>
                  <div style={{ font: "700 13px 'Inter'", color: t.textMuted }}>{row.fulfillment_branch || row.branch}</div>
                  <div style={{ font: "700 13px 'Inter'", color: t.textMuted }}>{row.current_branch}</div>
                  <div>
                    <div style={{ font: "700 12.5px 'Inter'", color: row.request_status === 'cancelled' ? RED : STATUS_COLOR[row.trackingStatus] || t.textMuted }}>
                      {row.request_status === 'cancelled' ? 'Request cancelled' : TRACKING_LABELS[row.trackingStatus]}
                    </div>
                    <div style={{ font: "600 11.5px 'Inter'", color: t.textFaint, marginTop: 3 }}>{row.currentStockStatusLabel}</div>
                  </div>
                  <div style={{ font: "600 12px 'Inter'", color: latest ? t.textMuted : t.textFaint }}>{latest?.movementLabel || 'Requested'}</div>
                  ...
                  <div style={{ padding: '0 16px 16px', background: t.bgSide, overflowX: 'auto' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(220px, 1fr))', gap: 9, padding: '12px 0 4px', minWidth: 760 }}>
                      <div style={{ padding: 10, background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 8 }}>
                        <div style={{ font: "800 11.5px 'Inter'", color: t.textFaint }}>DAILY EXCEL SNAPSHOT</div>
                        <div style={{ marginTop: 5, font: "700 13px 'Inter'", color: row.snapshot.active ? t.text : AMBER }}>
                          {row.snapshot.active ? `${row.snapshot.branch || 'Unknown branch'} · ${row.currentStockStatusLabel}` : 'Not in latest ERP snapshot'}
                        </div>
                        <div style={{ marginTop: 4, font: "500 11.5px 'Inter'", color: t.textFaint }}>
                      ...
                      <div style={{ padding: 10, background: t.bgCard, border: `1px solid ${row.snapshotReconciliation.state === 'mismatch' ? RED : t.border}`, borderRadius: 8 }}>
                        <div style={{ font: "800 11.5px 'Inter'", color: t.textFaint }}>SNAPSHOT RECONCILIATION</div>
                        <div style={{ marginTop: 5, font: "700 13px 'Inter'", color: row.snapshotReconciliation.state === 'mismatch' ? RED : row.snapshotReconciliation.state === 'reconciled' ? GREEN : t.text }}>
                          {row.snapshotReconciliation.label}
                        </div>
                      </div>
                      <div style={{ padding: 10, background: t.bgCard, border: `1px solid ${t.border}`, borderRadius: 8 }}>
                        <div style={{ font: "800 11.5px 'Inter'", color: t.textFaint }}>LIVE / CONFIRMED ERP</div>
                        <div style={{ marginTop: 5, font: "700 12.5px 'Inter'", color: row.erp_transfer_confirmed ? GREEN : t.textFaint }}>BT issued: {row.erp_transfer_confirmed ? 'yes' : 'no'}</div>
                        <div style={{ marginTop: 3, font: "700 12.5px 'Inter'", color: row.erp_transfer_received ? GREEN : t.textFaint }}>BT received: {row.erp_transfer_received ? 'yes' : 'no'}</div>
                        {row.liveErpVerification && <div style={{ marginTop: 3, font: "700 12px 'Inter'", color: GREEN }}>Availability rechecked live at {formatDate(row.liveErpVerification.verifiedAt)}</div>}
                        {row.request_status === 'cancelled' && <div style={{ marginTop: 3, font: "700 12px 'Inter'", color: RED }}>Rejected: {row.cancellation_status?.replaceAll('_', ' ') || 'unavailable'}{row.cancellation_reason ? ` · ${row.cancellation_reason}` : ''}</div>}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', padding: '12px 0' }}>
                      {[row.shape || row.item || row.category, row.carat ? `${row.carat} cts` : row.diamond_cts ? `${row.diamond_cts} d.cts` : null, row.color, row.clarity, row.lab].filter(Boolean).map((value) => (
                        <span key={String(value)} style={{ padding: '4px 7px', borderRadius: 5, background: 'oklch(78% 0.13 240 / 0.13)', color: ACCENT, font: "700 12px 'Inter'" }}>{value}</span>
                      ))}
                    </div>

                    <div style={{ font: "800 14px 'Inter'", color: t.text, marginBottom: 9 }}>Stone movement history</div>
                    <div style={{ minWidth: 1100, border: `1px solid ${t.border}`, borderRadius: 9, overflow: 'hidden', background: t.bgCard }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '145px 80px 155px 95px minmax(160px,1fr) 90px 90px 115px minmax(120px,1fr)', gap: 10, padding: '10px 12px', font: "800 11.5px 'Inter'", color: t.textFainter }}>
                      ...
                      {row.movements.map((event) => (
                        <div key={event.id} style={{ display: 'grid', gridTemplateColumns: '145px 80px 155px 95px minmax(160px,1fr) 90px 90px 115px minmax(120px,1fr)', gap: 10, padding: '11px 12px', alignItems: 'center', borderTop: `1px solid ${t.rowBorder}` }}>
                          <div style={{ font: "500 12.5px 'Inter'", color: t.textFaint }}>{formatDate(event.createdAt)}</div>
                          <div style={{ font: "700 12.5px 'Inter'", color: t.textMuted }}>#{row.request_id}</div>
                          <Copyable value={row.barcode} style={{ font: "700 13px 'JetBrains Mono'", color: ACCENT }} />
                          <div style={{ font: "700 12.5px 'Inter'", color: t.textMuted }}>{event.fromBranch || row.current_branch}</div>
                          <div style={{ font: "700 12.5px 'Inter'", color: event.movementType.includes('transfer') ? ACCENT : t.text }}>{event.movementLabel}</div>
                          <div style={{ font: "600 12.5px 'Inter'", color: t.textMuted }}>{event.fromBranch || '—'}</div>
                          <div style={{ font: "600 12.5px 'Inter'", color: t.textMuted }}>{event.toBranch || 'Customer'}</div>
                          <div style={{ font: "600 12.5px 'Inter'", color: row.cert_found ? ACCENT : t.textFaint }}>
                            {row.cert_found ? 'Confirmed' : row.cert_no ? <Copyable value={row.cert_no} /> : 'Pending'}
                          </div>
                          <div style={{ font: "600 12.5px 'Inter'", color: t.textMuted }}>{event.actorName}</div>
                        </div>
                      ))}
                    </div>
                  </div>
        ...
        {!loading && total > 0 && (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 12, color: t.textMuted, font: "500 14px 'Inter'" }}>
