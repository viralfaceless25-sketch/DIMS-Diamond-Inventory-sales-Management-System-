...
export default function RepLayout({ children }: { children: React.ReactNode }) {
  ...
  if (loading || !user) {
    return <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#0a0e0d', color: '#888', font: "500 14px 'Inter'" }}>Loading…</div>;
  }
        ...
        <div style={{ display: 'flex', width: '100%', height: '100vh', background: t.bg, color: t.text, overflow: 'hidden' }}>
          ...
          <div style={{ width: 224, flex: 'none', background: t.bgSide, borderRight: `1px solid ${t.border}`, display: 'flex', flexDirection: 'column', padding: '18px 14px' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 8px', marginBottom: 12, background: t.bgCard, borderRadius: 10, border: `1px solid ${t.border}` }}>
              <div style={{ width: 34, height: 34, borderRadius: '50%', background: ACCENT, display: 'flex', alignItems: 'center', justifyContent: 'center', font: "600 13px 'Inter'", color: '#0a0e0d', flex: 'none' }}>
                {initialsOf(user.name || user.email)}
              </div>
              <div style={{ minWidth: 0 }}>
                <div style={{ font: "600 12.5px 'Inter'", color: t.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{user.name || user.email}</div>
            ...
            {NAV.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href} style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', gap: 11, padding: '10px 12px', borderRadius: 9, marginBottom: 3, background: active ? 'oklch(78% 0.13 240 / 0.14)' : 'transparent' }}>
                  ...
                  {showBadge && <span style={{ marginLeft: 'auto', font: "700 10.5px 'JetBrains Mono'", background: 'oklch(75% 0.14 80)', color: '#0a0e0d', padding: '1px 7px', borderRadius: 9 }}>{cartCount}</span>}
                </Link>
              );
            })}
            ...
            {pathname === '/rep/request-stones' && (
              <div style={{ position: 'relative', margin: '12px 4px 0', paddingTop: 12, borderTop: `1px solid ${t.border}` }}>
                <div style={{ font: "700 10px 'Inter'", color: t.textFaint, letterSpacing: '0.05em', margin: '0 6px 9px' }}>FILTER STOCK</div>
                <div style={{ marginBottom: 10 }}>
                  <div style={{ font: "700 10.5px 'Inter'", color: t.textFaint, margin: '0 6px 5px' }}>Color</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4 }}>
                    {COLOR_ORDER.map((value) => <button key={value} onClick={() => toggleValue(colors, setColors, value)} style={{ border: `1px solid ${colors.includes(value) ? 'oklch(78% 0.13 240 / 0.35)' : 'transparent'}`, borderRadius: 5, padding: '5px 2px', cursor: 'pointer', background: colors.includes(value) ? 'oklch(78% 0.13 240 / 0.16)' : t.chipBg, color: colors.includes(value) ? ACCENT : t.textMuted, font: "700 10px Arial, sans-serif" }}>{value}</button>)}
                  </div>
                </div>
                <div style={{ marginBottom: 10 }}>
                  <div style={{ font: "700 10.5px 'Inter'", color: t.textFaint, margin: '0 6px 5px' }}>Clarity</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 4 }}>
                    {CLARITY_ORDER.map((value) => <button key={value} onClick={() => toggleValue(clarities, setClarities, value)} style={{ border: `1px solid ${clarities.includes(value) ? 'oklch(78% 0.13 240 / 0.35)' : 'transparent'}`, borderRadius: 5, padding: '5px 2px', cursor: 'pointer', background: clarities.includes(value) ? 'oklch(78% 0.13 240 / 0.16)' : t.chipBg, color: clarities.includes(value) ? ACCENT : t.textMuted, font: "700 10px Arial, sans-serif" }}>{value}</button>)}
                  </div>
                </div>
                <div>
                  <div style={{ font: "700 10.5px 'Inter'", color: t.textFaint, margin: '0 6px 5px' }}>Shape</div>
                  <button onClick={() => setShapePanelOpen((open) => !open)} style={{ width: '100%', border: `1px solid ${shapes.length ? 'oklch(78% 0.13 240 / 0.35)' : t.border}`, borderRadius: 6, padding: '7px 8px', cursor: 'pointer', background: shapes.length ? 'oklch(78% 0.13 240 / 0.12)' : t.bgCard, color: shapes.length ? ACCENT : t.textMuted, textAlign: 'left', font: "700 10.5px 'Inter'" }}>{shapes.length ? `${shapes.length} shape${shapes.length === 1 ? '' : 's'} selected` : 'Choose shapes'}</button>
                  {shapePanelOpen && <div style={{ position: 'absolute', zIndex: 20, top: '100%', left: 0, width: 500, marginTop: 8, padding: 12, border: `1px solid ${t.border}`, borderRadius: 8, background: t.bgCard, boxShadow: '0 12px 30px rgb(0 0 0 / 0.18)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 9 }}><span style={{ font: "700 11px 'Inter'", color: t.text }}>Shapes</span><button onClick={() => setShapes([])} style={{ border: 'none', background: 'transparent', color: t.textFaint, font: "600 10px 'Inter'", cursor: 'pointer' }}>Clear</button></div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 5 }}>{shapeOptions.map((value) => <button key={value} onClick={() => toggleValue(shapes, setShapes, value)} style={{ border: `1px solid ${shapes.includes(value) ? 'oklch(78% 0.13 240 / 0.35)' : t.border}`, borderRadius: 5, padding: '6px 5px', minHeight: 30, cursor: 'pointer', background: shapes.includes(value) ? 'oklch(78% 0.13 240 / 0.16)' : t.chipBg, color: shapes.includes(value) ? ACCENT : t.textMuted, font: "600 9.5px Arial, sans-serif", lineHeight: 1.1 }}>{value}</button>)}</div>
                  </div>}
                </div>
              </div>
            )}
